...
export const AppListItem = ({ app }: AppListItemProps) => {
  ...
  return (
    <Link href={`/dashboard/${app.id}`} className="w-full flex items-center justify-between px-[16px] py-[16px] border-b-2 border-b-foreground cursor-pointer group hover:bg-primary hover:text-primary-foreground transition-colors duration-500">
      <h1>{app.name}</h1>
